  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-2xl font-bold tracking-tight">Dietas</h1>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description="Cadastre pacientes para gerenciar suas dietas."
        />
      ) : (
        <div className="grid gap-3">
          {filtered.map((patient) => (
            <Link
              key={patient.id}
              href={`/pacientes/${patient.id}/dietas`}
              className="flex items-center justify-between rounded-lg border p-4 hover:bg-muted/50 transition-colors"
            >
              <div className="min-w-0">
                <p className="font-medium truncate">{patient.full_name}</p>
                <p className="text-sm text-muted-foreground">Ver dietas do paciente</p>
              </div>
